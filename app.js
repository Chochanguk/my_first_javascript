...
function onLoginBtnClick()
{
    ...
    //유효성검사를 해서 사용자 입력값 받기
    const username=loginInput.value; //우선 사용자의 입력값을 받음.
    console.log(username);
}
//loginButton.addEventListener("click",onLoginBtnClick); 
*/
/*form을 이용한 사용자입력 받기
1. 브라우저 기본 동작은 form을 submit 하거나 a태그의 link를 클릭하면 새로고침해버린다는 거다.
2. 따라서 해당 event의 기본동작을 막는 방법은 인자를 통해 가능하다.
...
loginForm.addEventListener("submit",onLoginSubmit); //submit 동작(기본 동작)시 새로고침을 방지하는 함수 호출

function handleLinkClick(event) //관행적으로 event라고 적음.
{
    console.log(event);
    event.preventDefault(); //브라우저의 기본동작을 만들어줌
    //alert("노마드코더 홈페이지로 이동하시겠습니까?");
}
//link.addEventListener("click",handleLinkClick); //클릭시 새로고침 방지
*/
/*Getting UserName(입력form은 숨기고, 입력한 내용은 보이게 하기1)
...
function onLoginSubmit(event) //관행적으로 event라고 적음.
{
    ...
    loginForm.classList.add("hidden"); //form을 사라지게 하는 css 추가
    greeting1.innerText="Hello " + username; //헤딩 태그의 내용 수정(자바스크립트 방식)
    greeting2.innerText=`Hello ${username} 조창욱`; //헤딩 태그의 내용 수정(`(백틱):키보드 1왼쪽)
...
*/
/*Saving UserName1(localStorage 메소드 사용,localStroage는 Application 탭에서 확인 가능)
1. localStroage.setItem("key","value"); //저장
